...
func TestHandleOdd(t *testing.T) {
	...
	// Assert
	if echo == nil {
		t.Errorf("data: 1, but received nil event from is-odd")
	} else {
		if string(echo.Data()) != "1" {
			t.Errorf("the received event expected data to be '1', got '%s'", echo.Data())
		}
	}
}
...
func TestHandleEven(t *testing.T) {
	...
	// Assert
	if echo != nil {
		t.Errorf("data: 2, but received non-nil event from is-odd")
	}
}
